...
SELECT
    ...
    last_name
FROM customer 
WHERE DATEDIFF(create_date, last_update) >= 10; #날짜를 비교하는 DATEDIFF 함수 사용

# 1-1. customer 테이블에서 10일 이상의 영화를 대여한 고객들의 first_name, last_name을 조회
SELECT
    c.first_name,
    c.last_name
FROM customer AS c
INNER JOIN rental AS r ON c.customer_id = r.customer_id
WHERE DATEDIFF(return_date, rental_date) >= 10; #rental테이블의 대여 날짜와 반납 날짜를 비교하는 DATEDIFF 함수 사용
